...
function View() {
	...
	this.renderContent = function (hashPageName) {
		...
		modalHeaderRegistration = myContainer.querySelector(
			".modal-header.register"
		);
		modalButtonRegistration = myContainer.querySelector(
			".form-submit-register"
		);
		modalBlockRegistration = myContainer.querySelector(".form-register__block");
		successfulRegistrationBlock = myContainer.querySelector(
		...
		exercisesSelectWrapper = myContainer.querySelector(
			".training-modal__exercisename"
		);
	...
	this.ifError = function (errorCode) {
		if (
			...
			formError.innerHTML =
				"Неверный логин или пароль. Проверьте правильность ввода. Пароль должен быть не менее 6 символов.";
		} else if (
	...
	this.renderInfo = function (user) {
		if (userInfoWrapper) {
			userInfoWrapper.innerHTML = UserInfo.render(
				user.fullName,
				user.birthday,
				user.gender,
				user.weight,
	...
	this.addExercise = function (
		...
		if (trainingWrapper) {
			trainingWrapper.innerHTML += Exercise.render(
				exerciseName,
				exerciseSet,
				exerciseWeight,
				exerciseTime,
		...
		document
			.querySelector(".mainblock__header")
			.classList.add("active-training");
		document
			.querySelector(".mainblock__content")
			.classList.add("active-training");
	};
		...
		if (trainingWrapper) {
			for (let key in exercise) {
				...
				trainingWrapper.innerHTML += Exercise.render(
					...
					exerciseKey.exerciseSet,
					exerciseKey.exerciseWeight,
					exerciseKey.exerciseTime
				);
			}
		}
	};
	...
	this.clearTrainingBlock = function () {
		...
		document
			.querySelector(".mainblock__body")
			.classList.remove("active-training");
		document
			.querySelector(".mainblock__header")
			.classList.remove("active-training");
		document
	...
	this.renderProducts = function (products) {
		if (productsWrapper) {
			for (let key in products) {
				...
				productsWrapper.innerHTML += ProductBlock.render(
					productKey.product,
					productKey.protein,
					productKey.fat,
					productKey.carbohydrates,
					productKey.calories
				);
			}
		}
	...
	this.renderExercises = function (exercises) {
		if (exercisesWrapper) {
			for (let key in exercises) {
				...
				exercisesWrapper.innerHTML += ExerciseBlock.render(
					exercise.title,
					exercise.image,
